...
function loadConfig(): AppConfig {
  ...
  if (!baseUrl) {
    console.error(
      "❌  BOOKSTACK_BASE_URL is required.\n" +
  ...
  if (!apiToken) {
    console.error(
      "❌  BOOKSTACK_API_TOKEN is required.\n" +
  ...
  try {
    ...
    console.error(
      `❌  BOOKSTACK_BASE_URL is not a valid URL: "${baseUrl}"\n` +
...
async function main(): Promise<void> {
  ...
  const server = new Server(
    { name: "@h4sht/bookstack-mcp", version: "1.0.0" },
    { capabilities: { tools: {} } }
  );
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    if (!tool) {
      ...
      throw new Error(
        `Unknown tool: ${name}. Available: ${[...toolMap.keys()].sort().join(", ")}`
      );
    }
